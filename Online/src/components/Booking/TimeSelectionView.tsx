...
import { IRecord } from '../../Models/AllModels';
...
export interface TimeSelectionViewProps {
    ...
    onChanged: (selectedTime?: string) => void;
}
...
export function TimeSelectionView(props: TimeSelectionViewProps) {
    ...
    const [selectedTime, setSelectedTime] = React.useState(selectedTimePoint);
    
    const handleOnClick = (timePoint: moment.Moment) => {
        setSelectedTime(timePoint.toISOString())
        onChanged(timePoint.toISOString());
    };

    // Переводим iso-строки в moment
    const adjustedTimeSlotList = timePoints?.map(o => moment(o, "LT")) ?? [];
    const adjustedSelectedTimePoint = moment(selectedTime)
    ...
    return (
        <Grid container spacing={2} justifyContent="center" alignItems="center">
            {adjustedTimeSlotList?.map((timePoint, index) => (
                <Grid key={index} item minWidth={100}>
                    <TextField 
                        type="button"
                        className={timePoint.isSame(adjustedSelectedTimePoint) ? classes.selectedTimePoint : undefined}
                        color="secondary"
                        size="small"
