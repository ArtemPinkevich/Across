...
public class SearchOrderByIdQueryHandler : IRequestHandler<SearchOrderByIdQuery, TransportationOrdersListDto>
{
    private readonly IMapper _mapper;
    private readonly IRepository<Entities.TransportationOrder> _ordersRepository;
    
    public SearchOrderByIdQueryHandler(IRepository<Entities.TransportationOrder> ordersRepository, IMapper mapper)
    {
        _mapper = mapper;
        _ordersRepository = ordersRepository;
    }
    
    public async Task<TransportationOrdersListDto> Handle(SearchOrderByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        if (transportationOrders == null || transportationOrders.Count <= 0)
        {
            return new TransportationOrdersListDto()
            {
                ...
                {
                    Result = ApiResult.Failed, Reasons = new[] { $"no orders found with id {request.OrderId}" }
                }
            };
        }
